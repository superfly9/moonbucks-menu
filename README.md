...
- [x] 메뉴의 수정 버튼을 눌러 메뉴 이름 수정할 수 있다. => 이벤트 위임을 쓸 것인지 / 이벤트 리스너는 dom생성시 template string에 넣을 것인지, 추후 addEvent할 것인지
  - [x] 메뉴 수정시 브라우저에서 제공하는 `prompt` 인터페이스를 활용한다.
- [x] 메뉴 삭제 버튼을 이용하여 메뉴 삭제할 수 있다.
  - [x] 메뉴 삭제시 브라우저에서 제공하는 `confirm` 인터페이스를 활용한다.
- [x] 총 메뉴 갯수를 count하여 상단에 보여준다.=> 부모.children.length 할 것인지 혹은 li갯수 querySelectorAll로 처리할 것인지
